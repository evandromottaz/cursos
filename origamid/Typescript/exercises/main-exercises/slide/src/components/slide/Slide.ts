...
function isVideo(el: Element | null): el is HTMLVideoElement {
	return el instanceof HTMLVideoElement;
}

type TypeTimeoutMethod = () => void;
type TypeTimeout = {
	id: number;
	clear: TypeTimeoutMethod;
	proceed: TypeTimeoutMethod;
	pause: TypeTimeoutMethod;
} | null;
...
interface ISlide {
	...
	index: number;
	timeout: TypeTimeout;
	isPausedTimeout: TypeTimeout;
	isPaused: boolean;
	isContinued: boolean;
	thumbItems: null | HTMLElement[];
	thumbCurrent: null | HTMLElement;
}
...
function Slide(
	...
	function setDurationVideo(video: HTMLVideoElement) {
		video.muted = true;
		video.play();
		let firstPlay = true;
	...
	function setAutoplay(time: number) {
		slide.timeout?.clear();
	...
	function ProgressBar() {
		...
		slide.thumbItems = [
			...container.querySelectorAll<HTMLElement>(".slide__progress-bar"),
		];
	}
