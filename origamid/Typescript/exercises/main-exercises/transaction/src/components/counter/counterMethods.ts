...
import { PaymentMethod, PortugueseDay, SalesPerDays, Status } from "./typing";

export function setValueBRL(valueBRL: string) {
...
export function setSalesPerDays(date: string) {
...
export function setDayMostSale(salesPerDays: SalesPerDays) {
	const [firstSale] = Object.entries(salesPerDays).sort((a, b) => b[1] - a[1]);
	const portugueseDay: PortugueseDay = {
		sunday: "Domingo",
	...
	const [day] = firstSale;
	if (portugueseDay[day]) counterData.dayMostSale = portugueseDay[day];
}
