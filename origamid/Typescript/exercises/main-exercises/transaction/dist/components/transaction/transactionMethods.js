...
export function getNewTransactions(transactions) {
...
export function getTransactionHead(transactions) {
    const { thead } = Table();
    if (!transactions || !Array.isArray(transactions))
        return thead;
    ...
    if (isTransaction(firstTransaction)) {
        const [status, id, date, name, paymentMethod, email, valueBRL, newClient] = Object.keys(firstTransaction);
...
export function getTransactionBody(transactions, columns) {
    const { tbody } = Table();
    const trs = transactions.map((transaction) => {
        ...
        Object.entries(transaction).forEach(([key, value]) => {
            ...
            if (hasCol)
                frag.querySelectorAll("td")[colIndex].innerText = value;
        });
        const { tr } = Table();
        tr.append(frag);
        return tr;
...
//# sourceMappingURL=transactionMethods.js.map